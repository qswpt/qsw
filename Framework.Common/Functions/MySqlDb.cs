...
namespace Framework.Common.Functions
{
    public class MySqlDb
    {
        ...
        private const int ALARM_THRESHOLD_VALUE = 500000; // 50毫秒
        ...
        public int ExecuteNonQuery(string sql)
        {
            ...
            {
                LogUtil.Warn($"执行SQL超过预定伐值:{ALARM_THRESHOLD_VALUE / 10000}(毫秒), SQL:{sql}, Mileseconds:{elapsedTicks / 10000}");
            }
        ...
        public int ExecuteNonQuery(string sql, Dictionary<string, object> p)
        {
            ...
            {
                LogUtil.Error(ex.ToString() + "|" + sql + "|" + JsonUtil.Serialize(p));
                throw new Exception(ex.Message, ex);
            }
            finally
            {
        ...
        private MySqlConnection GetConnection()
        {
            ...
            bool ok = _queue.TryDequeue(out connection);
            if (!ok)
            {
            ...
            {
                LogUtil.Error(ex.ToString() + "|" + _connectionString);
                throw new Exception(ex.Message, ex);
            }
            ...
            if (elapsedTicks > ALARM_THRESHOLD_VALUE)
            {
                LogUtil.Warn($"打开连接超过预定伐值:{ALARM_THRESHOLD_VALUE / 10000}(毫秒), Queue Count:{_queue.Count}, ConnectionString:{_connectionString}, Mileseconds:{elapsedTicks / 10000}");
            }
