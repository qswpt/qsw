        ...
        if (data.Data.length > 0) {
            for (var i = 0; i < data.Data.length; i++) {
                html += '<div style="width:100%; position:absolute; top:' + topNum + 'rem; height:6.2rem; background-color:#fff border-radius: 0.5rem;" onclick="openInfo(' + data.Data[i].CommodityId + ')">' +
                    ...
                    ' font-size:0.7rem; color:#999999;">颜色：' + data.Data[i].CommodityGeneral + '</span><span style="position:inherit; left:0.25rem; top:2.67rem; font-size:0.7rem; color:#999999;">品牌: ' + data.Data[i].BrandName + '</span>' +
                    ...
                    ' font-size:0.7rem; color:#999999;">价格:<span style="color:red;">￥' + (data.Data[i].CommoditySpec * data.Data[i].CommodityPrice).toFixed(2) + '</span>元</span><span style="position:inherit; right:0.5rem; top:1.5rem; font-size:12px; color:#999999;">索引号: ' + data.Data[i].CommodityIndex + '</span>' +
                    '<span style="position:inherit; right:0.5rem; top:2.67rem; font-size:12px; color:#999999;">商品编码: ' + data.Data[i].CommodityCode + '</span>' +
