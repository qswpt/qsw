    ...
    if (!isPasLogin(data)) {
        ...
        if (data != null && data.Data.length > 0) {
            for (var i = 0; i < data.Data.length; i++) {
                ...
                var isSample = data.Data[i].IsSample;
             ...
             '<span style="position:absolute; right:18px; color:#ff6a00;">' + wapState(data.Data[i].OrderState, isSample) + '</span></div>';
                var dtl = data.Data[i].OrdrList;
                var dtlhtml = '';
                for (var x = 0; x < dtl.length; x++) {
                    var hej = isSample == 0 ? (dtl[x].CommodityPrice * dtl[x].CommoditySpec * dtl[x].CommNumber) : 0;
                    ...
                    '<span style="position:absolute;  right:12px; top:68px; font-size:12px;">合计：<span style="color:red;">￥' + hej + '</span></span>' +
               ...
               '<div style="position:inherit; width:100%; height:30px; background-color:#fff;">' +
                  '<span style="position:absolute; left:1.5rem; top:3px;">运单号：' + exCode + '</span>' +
               ...
               '<div style="position:absolute; right:100px; top:0px; text-align:center; border-radius: 0.6rem; width:75px; height:23px; border:solid 1px #696767; color:#696767;" onclick="showMove(' + data.Data[i].OrderId + ');">取消订单</div>' +
...
function wapState(state, IsSample) {
    var str = '';
    if (IsSample == 0) {
        switch (state) {
            case 0:
